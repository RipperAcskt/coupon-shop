...
func (r *subscriptionCouponRepo) GetOrgSubscriptionLevel(email string) (int, error) {
	orgLevel := 0
	err := r.database.QueryRow("WITH subscription(organization_id) AS (SELECT organization_id FROM members WHERE email = $1) SELECT level_subscription FROM organization JOIN subscription ON subscription.organization_id = organization.id ORDER BY level_subscription DESC", email).Scan(&orgLevel)
	if err != nil {
		return 0, err
	}
	return orgLevel, nil
